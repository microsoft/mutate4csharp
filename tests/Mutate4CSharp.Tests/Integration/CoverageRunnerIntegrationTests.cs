...
/// sample (a production project plus an xUnit test project wired with <c>coverlet.collector</c>) via
/// <see cref="TestProjectFactory"/> into a unique temp directory, drives the REAL
/// <see cref="CoverageRunner"/> over a real <see cref="ProcessCommandExecutor"/> — which runs
/// <c>dotnet test --collect:"XPlat Code Coverage" -p:DeterministicSourcePaths=false</c> — and asserts
/// that the produced coverlet Cobertura report reconciles against on-disk paths through
/// <see cref="CoberturaLineCoverageParser"/>: a line the single test executes resolves to an A4 key
/// that reads covered (hits &gt; 0), a line it never executes reads uncovered, and the DD2(b)
/// executed-test count is positive. Unlike the synthetic-XML unit tests (<c>CoverageRunnerTests</c>,
/// <c>CoberturaLineCoverageParserTests</c>), this proves the coverlet → Cobertura → A4-keying adapter
/// works on a live run — the reconciliation Anders' T8 review flagged as the one thing synthetic XML
/// cannot verify. There is no Java oracle (mutate4java used JaCoCo); this validates the C#/coverlet
/// ecosystem adapter, which is the point of slice S6. The class is tagged
/// <c>[Trait("type", "IntegrationTests")]</c> so DD3 excludes it from the fast unit run and it stays
...
public sealed class CoverageRunnerIntegrationTests : IDisposable
{
    ...
    public void RealCoverletRunReconcilesCoveredAndUncoveredLines()
    {
        ...
        ModuleResolution module = ModuleResolution.Resolved(
            _project.ProjectName, _project.ProjectFile, _project.TestProjectFile);
        ...
        string key = CoberturaLineCoverageParser.NormalizeSourcePath(_project.ProductionFile("Calculator.cs"));

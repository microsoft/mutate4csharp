...
**Slices:** S1 ✅ · S2 ✅ · S3 (selection/coverage/report) ✅ · S4 (exec/workers) ✅ · S5 (engine + CLI wiring) ✅ — **the tool is fully wired and runnable end-to-end (191 tests green).** S6 (integration/acceptance) in progress: T16 ✅ (real coverlet reconciliation IT); suite now **192 (191 unit + 1 IT)**.
- **Carry-forward — T17/T18 sample-gen (Anders, T16 review):** keep hermetic-sample generation **inline through T17**; **extract a shared `TestProjectFactory` at T18** (Rule-of-Three — all three shapes visible by then). To prevent warm-cache drift, optionally share **only** the version-pin constants (Test.Sdk 17.8.0 / xunit 2.5.3 / runner.visualstudio 2.5.3 / coverlet.collector 6.0.0) + a `WriteHermeticGuards(root)` helper early. Don't build the factory speculatively.
- **Carry-forward — T17 fidelity hardening (Anders, T16 review, non-blocking):** (a) **R-D flag pin:** T16 proves the happy path but NOT the *necessity* of `-p:DeterministicSourcePaths=false` (the hermetic sample never sets `ContinuousIntegrationBuild=true`, so the `/_/` remap is never triggered → the test stays green even if the flag is removed). Add a variant that writes `<ContinuousIntegrationBuild>true</ContinuousIntegrationBuild>` and asserts reconciliation still holds (ideally fails without the flag). (b) **DD3 anti-fan-out:** T16's single-project sample doesn't stress DD3 — add a sibling project or root `.sln` so the explicit-target + cwd-in-test-project-dir scoping is proven to PREVENT the whole-solution fan-out `CoverageRunner` warns about.
- **CI gating (Anders T16 review, recommend B — PENDING Mr. Das):** split CI `dotnet test` into a **fast unit step** (`--filter "type!=IntegrationTests"`, keep `--collect`) that gates the build + a **separate IT step** (`--filter "type=IntegrationTests"`, **drop `--collect`**), two steps in one job (share restore+build, `--no-build`). Rationale: signal isolation; avoids nested-`--collect` collector interference (outer XPlat + IT's own coverlet); S6 must run on the **Linux** runner because A4's `ToUpperInvariant` folding is Windows-only (the cross-platform seam the ITs exist to catch). Option C (unit-only in CI) rejected for that reason. Escalated to Mr. Das as infra policy.

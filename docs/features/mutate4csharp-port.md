...
**Slices:** S1 ✅ · S2 ✅ · S3 ✅ · S4 ✅ · S5 ✅ · **CI split (Option B, ruled by Mr. Das) ✅** — fast unit gate + isolated IT step, green on Linux. S6 (integration/acceptance) in progress: **T16 ✅** (real coverlet reconciliation IT) · **T17 ✅** (worker-scoping keystone — mutant KILLED / survivor SURVIVES via real `dotnet test`, causal R-C leak-guard, DD3 anti-fan-out). Suite now **196 (192 unit + 4 IT)**. Remaining in S6: **T18** (acceptance — spawn the exe end-to-end + extract `TestProjectFactory`).
- **Carry-forward — T18 acceptance (Anders, T17 review):** (1) **extract the full `TestProjectFactory` now** — Rule-of-Three satisfied (T16+T17+T18); fold in `HermeticSample`; generalize to production+test project with traits, optional stray/`.sln`, source files. (2) **Spawn-the-exe mechanics:** locate the Release build output deterministically; child cwd = generated sample root (Program uses `Directory.GetCurrentDirectory()` as workspace root); **capture stdout+stderr on separate async drains** in the harness (the T11 drain-before-wait lesson applies to your OWN process capture or you deadlock on a chatty nested `dotnet test`). (3) **Assert both contracts precisely:** stdout **§13 report strings byte-verbatim** (`\n`), stderr separately; **§14 exit codes 0/1/2/3** — must hit the **DD2 exit-2 arms** (no owning project / no test project / zero unit tests) AND the **exit-3 survivor**, not just the happy path. (4) **Determinism:** use a **multi-mutant** sample so the verbatim §13 assertion also pins result **ordering** (job order via `WorkerFutureCollector` → `ExecutionOutcomeWriter`); a single-mutant sample won't catch an ordering flake. (5) **Optional:** a `--test-command` acceptance case pins the minor T15 cwd inconsistency (baseline cwd = test-project dir vs worker cwd = repo-root copy).
- **Carry-forward — T18 version-drift guard (Anders, T17 review, non-blocking):** `HermeticSample`'s four package-version constants are **copies** of `Mutate4CSharp.Tests.Common.targets`, synced only by a comment. When `TestProjectFactory` lands, either read the pins from the `.targets` or add a guard test that parses `Mutate4CSharp.Tests.Common.targets` and asserts the four versions match (else a common-targets bump silently causes a warm-cache miss).
- **Carry-forward — T18 R-D flag necessity pin (Anders, T16 review, still open, optional):** T16 proves the coverlet happy path but NOT the *necessity* of `-p:DeterministicSourcePaths=false` (the hermetic sample never sets `ContinuousIntegrationBuild=true`, so the `/_/` remap never fires → the test stays green even if the flag is removed). A variant that writes `<ContinuousIntegrationBuild>true</ContinuousIntegrationBuild>` and asserts reconciliation still holds (ideally fails without the flag) would pin it. (T17's DD3 anti-fan-out carry-forward is **DONE** — implemented as the `ScopedTestRunDoesNotFanOutToCompetingSolution` fact.)
